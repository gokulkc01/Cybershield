"""Build mixed training splits from labeled session NPZ files."""
...
from src.features.feature_config import FEATURE_DIM, LABEL_BENIGN, LABEL_C2, SESSION_LEN, VAL_SPLIT
...
def mix_sessions(
    ...
    """
    if not 0 < c2_ratio < 1:
        raise ValueError(f"c2_ratio must be between 0 and 1, got {c2_ratio}")
    ...
    n_target = int(n_c2 / c2_ratio)
    n_benign = min(n_target - n_c2, len(benign_sessions))

    if n_benign <= 0:
        raise ValueError("Not enough benign sessions available to satisfy the requested c2_ratio")
